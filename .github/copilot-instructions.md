...
- 新規の UI を実装する場合は、必ず基底コンポーネントを使用してください。
- 新規の基底コンポーネントが必要になった場合は、src/types/design-tokens から各種デザイントークンを import して、それを TailwindCSS のユーティリティクラスにマッピングする形式で実装してください。
...
- データの追加・更新・削除処理では、必ず revalidatePath または revalidateTag を使用してキャッシュを更新すること。

- any 型の使用は原則として禁止します。
- 新たに React コンポーネントを作成する場合は、クラスコンポーネントではなく関数コンポーネントにしてください。
...
- ソースコードを変更する際は、指示された内容以外のコードは修正しないでください。指示されていないデザイン修正、リファクタリングは行わないでください。
